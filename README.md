...
## ⏱️ Искусственная задержка ответа

Сервер поддерживает искусственную задержку ответа для тестирования производительности. Поддерживаются два способа активации задержки:

### 1. Задержка через query параметр

Добавьте параметр `slow` в запрос:
- `?slow` - задержка в 1 секунду (по умолчанию)
- `?slow=2` - задержка в 2 секунды
- `?slow=0.5` - задержка в 500 мс

Пример:
```bash
http://127.0.0.1:8000/entities?slow=2
```

### 2. Задержка через тело запроса

В JSON теле запроса добавьте флаг `"SlowPerformance": true`. Можно также указать продолжительность задержки с помощью параметра `"DelaySeconds"`:
- `"SlowPerformance": true` - задержка в 1 секунду (по умолчанию)
- `"SlowPerformance": true, "DelaySeconds": 2` - задержка в 2 секунды
